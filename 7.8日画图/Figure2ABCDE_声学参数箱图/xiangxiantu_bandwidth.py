...
# 从Excel文件读取数据
df = pd.read_excel('1到10只bandwidth.xlsx', engine='openpyxl')
# 获取不同分类的数量
...
for i, (category, cat_df) in enumerate(df.groupby('Category')):
    ...
    box = ax.boxplot(box_data, positions=[pos], whiskerprops=dict(linestyle='--', linewidth=2),
                     ...
                     flierprops=dict(markerfacecolor='none', marker='o', markeredgecolor="#3c7ba4", markeredgewidth=2,
                                     markersize=10))
...
plt.ylabel('FM bandwidth (kHz)', fontsize=38)
...
# 保存图像
plt.savefig('1-10只bandwidth.jpg', dpi=300, bbox_inches='tight')
#plt.savefig('1-10只bandwidth.eps', dpi=300, bbox_inches='tight', pad_inches=0)
'''
def rgb_to_hex(rgb):
